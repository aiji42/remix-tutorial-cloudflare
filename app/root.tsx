...
const SideBar: VFC = () => {
  const { user } = useLoaderData<{ user: {name: string, playlists: Playlist[]} }>();
  return (
    <div className="sidebar bg-gray-900-spotify w-48 flex-none flex flex-col justify-between font-semibold">
      <ul className="py-6">
        <li className="border-l-4 border-green-600"><Link to="/" className="flex items-center mx-4 mt-4 group">
          ...
          <span className="ml-2 text-white group-hover:text-white">Home</span></Link></li>
        ...
        <ul className="leading-extra-loose mb-6">
          {user.playlists.map(({ id, name }) => (
            <li className="truncate" key={id}>
              <Link to={`/playlist/${id}`} className="hover:text-white">{name}</Link>
            </li>
          ))}
        </ul>
      </div>
