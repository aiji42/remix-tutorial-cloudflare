    ...
    <div className="text-gray-300 min-h-screen p-10">
      <div className="flex">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="text-gray-600 text-sm">Created by <a>{data.user.name}</a> - {data.songs.length} songs, {timeFormattedString(data.songs.reduce((res, { length }) => res + length, 0))}</p>
        </div>
      </div>
      ...
      <div className="mt-10">
        ...
        {data.songs.map((song) => (
          <div key={song.id} className="flex border-b border-gray-800 hover:bg-gray-800">
            ...
            <div className="p-3 w-full">{song.artist.name}</div>
            <div className="p-3 w-full"><Link to={`/album/${song.album?.id}`}>{song.album?.name}</Link></div>
